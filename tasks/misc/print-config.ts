...
task('print-config', 'Inits the DRE, to have access to all the plugins')
  .addParam('dataProvider', 'Address of ViniumProtocolDataProvider')
  ...
  .setAction(async ({ pool, dataProvider }, localBRE) => {
    ...
    const protocolDataProvider = await getViniumProtocolDataProvider(dataProvider);
    ...
    const tokensFields = ['viToken', 'stableVdToken', 'variableVdToken'];
